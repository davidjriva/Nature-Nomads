...
const limiter = rateLimit({
  ...
  windowMs: 60 * 60 * 1000,
  message:
    "Too many requests are coming from this IP, please try again in an hour.",
  keyGenerator: (req) => req.ip,
  skip: (req) => req.app.get("trust proxy") === false,
});
app.use("/api", limiter);
...
app.use(
  hpp({
    whitelist: [
      ...
      "difficulty",
      "price",
    ], // properties where duplicates are allowed in query string
  })
);
